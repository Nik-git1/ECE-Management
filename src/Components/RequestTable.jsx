...
const RequestTable = (props) => {
  const { allocatedEquipment, requestedEquipment, returnEquipment } =
  ...
  const getActionButton = (name) => {
    if (name === "borrow") {
      return {
        label: "Approve",
        color:
          "bg-green-500 text-white px-2 py-1 rounded-md flex items-center ml-2",
      ...
      return {
        label: "Send Reminder",
        color:
          "bg-yellow-500 text-white px-2 py-1 rounded-md flex items-center ml-2",
        action: () => console.log("allocate"),
      };
    }
    return null;
